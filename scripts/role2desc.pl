...
#
=head1 Expand Roleids to row descriptions

Given a table with roleIds, add a new column
containing role descriptions
...
while (my @tuples = ScriptThing::GetBatch($ih, undef, $column)) {
    ...
    my $i = 0;
    foreach my $roleId (@ids) {
        my $desc = $shrub->role_id_to_desc($roleId);
	print $lines[$i],"\t",$desc, "\n";
    }
}
