...
=head1 Find All Roles for a function

    svc_function_to_roles.pl [ options ]

Locate all of the roles associated with a specific function.

=head2 Parameters

See L<ServicesUtils> for more information about common command-line options.

The standard input must be tab-delimited and contain Function IDs in one column. The IDs of the features will
...
while (my @batch = ServicesUtils::get_batch($ih, $opt)) {
    my $resultsH = $helper->function_to_roles([map { $_->[0] } @batch], $opt->priv);
    # Output the batch.
